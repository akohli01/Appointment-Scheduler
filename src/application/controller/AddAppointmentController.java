...
public class AddAppointmentController {
    ...
    @FXML
    private AnchorPane addAppointmentPane;
    ...
    private final ReadOnlyObjectWrapper<Appointment> addAppointment = new ReadOnlyObjectWrapper<>();

    //Returns the appointment object
    public ReadOnlyObjectProperty<Appointment> addedAppointment() {
        return addAppointment.getReadOnlyProperty();
    }
    ...
    public void createAppointment() {
        ...
        if (checkValidTime(appointmentStartTime, appointmentEndTime) == true) {
            if (checkForOverlappingAppointments(appointmentDate, appointmentStartTime, appointmentEndTime) != true) {
                ...
                addAppointment.set(newAppointment);
    ...
    //Makes sure appointments do not overlap
    public boolean checkForOverlappingAppointments(LocalDate date, LocalTime startTime, LocalTime endTime) {
        ...
        try {
            PreparedStatement prepStatement = dbConnection.getConn().prepareStatement(
                    ...
                    + "FROM appointment WHERE (start BETWEEN ? AND ?) OR (end BETWEEN ? AND ?);");
            ...
            System.out.println("Something else is wrong with the code");
            e.printStackTrace();
        }
    ...
    private boolean isNull() {
        ...
        Set<Node> nodes = addAppointmentPane.lookupAll(".text-field");
        for (Node node : nodes) {
